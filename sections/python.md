...
- Try to open the Anaconda Prompt (On Mac, press <kbd>command</kbd> + <kbd>space</kbd> and type `Anaconda`. On Windows, press the <kbd>windows</kbd> key and type `Anaconda`). If no option comes up, you most likely don't have it installed.
...
- Type `conda update -n root conda` and press <kbd>enter</kbd>.
- Type <kbd>Y</kbd> (for yes) and press <kbd>enter</kbd> when asked.
- Wait for the process to finish. It takes some time.

- When the process finishes, type `conda update --all` and press <kbd>enter</kbd>.
- Type <kbd>Y</kbd> (for yes) for yes and press <kbd>enter</kbd> when asked.
...
3. You can ignore this. Just close this screen and wait for your download to finish.
...
Open the [macOS](osx_terminal.md) terminal and type `python`. The terminal should print something like:
